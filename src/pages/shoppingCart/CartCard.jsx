...
import { quantityChange, toggleItem } from '../../store/userSlice.js';

const CartCard = ({ product, quantity, isChecked }) => {
  const dispatch = useDispatch();

  const handleToggleItem = () => {
    dispatch(toggleItem({ productId: product.id }));
  };
  ...
  const isFreeShipping = quantity >= 3;
  return (
      ...
      <li>
          ...
          <div className="w-full flex gap-3 flex-col">
              ...
              <a className="flex">
                ...
                <div className="mx-3">
                  ...
                  <span className="flex text-xs text-gray-400 mt-1">
                    <p className="pr-2 border-r">3개 이상 주문시 무료배송</p>
            ...
            <div className="w-full flex justify-between">
              <div className="h-8 py-1 px-2 flex gap-1 border rounded-md bg-white">
                ...
                />
                <button onClick={plusQuantityHandler}>
                  <PlusIcon className="h-5 w-5 text-gray-700" />
                </button>
              </div>
              <span className="font-bold text-lg">
                {Math.round(sellingPrice * quantity).toLocaleString()}원
              </span>
            </div>
          </div>
        </div>
        <div className="border-t py-3">
          <div className="w-full inline-flex gap-1 flex-col justify-center items-center">
            {isFreeShipping ? (
              <span className="text-sm">배송비 무료</span>
            ) : (
              <>
